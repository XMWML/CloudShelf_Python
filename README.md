...
- 远端目录浏览、双击进入、返回上级和刷新；列表支持勾选多项或常规多选，`Ctrl/Command-A` 可全选或清除当前目录
...
- 连接状态显示和显式连接/断开控制
- 设置窗口集中管理文本预览、传输并发/下载目录和所选连接的同步规则
- 内置文本预览，支持可配置后缀与大小上限；二进制媒体预览不依赖平台专用组件
- 单项暂停/继续/取消/重试，以及传输队列批量开始、停止、失败重试和完成清理
- 连接或加载远端目录、下载预览时显示活动状态；可指定一个已保存的服务器在启动时自动连接
...
服务器地址、端口、用户名、密码和远端根目录在连接窗口中填写。也可直接输入包含协议、端口和根路径的 URL，例如 `sftp://files.example.com:22/backups` 或 `https://dav.example.com/files`；应用会识别并保存连接信息。
...
- Python 版本覆盖协议访问、浏览、文件操作、同步、传输管理、文本预览和可选拖拽上传。
- 不迁移 AppKit、Finder 挂载/拖放导出、Keychain、macOS 本地网络授权、原生媒体/PDF 预览和 macOS 专属窗口菜单行为。
...
- 设置中可配置 1 到 8 个传输并发数、默认下载目录、每次下载时询问目标位置，以及启动时自动连接的服务器；新并发数会在下次启动时用于新的工作线程池。后台错误记录在 `~/.cloudshelf/cloudshelf.log`。
